...
# dataset의 0번부터 2번 column 인덱스까지의 배열을 feature로 저장합니다.
feature = dataset[:,:3] # X
# dataset의 마지막 column 인덱스의 column 벡터를 label로 저장합니다.
label = dataset[:,3] # y

# 전체 데이터를 학습 데이터와 검증 데이터로 나눕니다.
# 학습 용 데이터의 크기를 0.25로 하여 전체 데이터를 분리합니다. random_state는 121을 입력합니다.
...
# shuffle을 하지 않고 test_size를 0.3으로 하여 분리합니다. random_state는 121을 입력합니다.
X_train_2, X_test_2, Y_train_2, Y_test_2 = train_test_split(feature, label, test_size=0.3, shuffle=False, random_state = 121) # 데이터 분할
